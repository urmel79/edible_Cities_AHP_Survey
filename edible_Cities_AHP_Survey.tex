\documentclass [oneside,10pt,a4paper,ngerman,BCOR10mm,headsepline,parindent,final]{scrartcl}
    ...
    % Document parameters
    % Document title
    \title{\textbf{\textsf{Introducing a procedure to perform the Analytic Hierarchy Process (AHP) with data from an own survey obtained from the \emph{SoSci Survey} platform using the R package \emph{ahpsurvey}}}}\author[1]{Bj\"orn Kasper (\href{mailto:kasper.bjoern@bgetem.de}{kasper.bjoern@bgetem.de})}
...
\makeatother
    ...
    \hypersetup{
      ...
      pdftitle=Introducing a procedure to perform the Analytic Hierarchy Process (AHP) with data from an own survey obtained from the \emph{SoSci Survey} platform using the R package \emph{ahpsurvey},
      colorlinks=true,
      urlcolor=urlcolor,
      linkcolor=linkcolor,
      citecolor=citecolor,
